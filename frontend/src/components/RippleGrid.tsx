...
// Define the structure for the shader uniforms
interface Uniforms {
  iTime: { value: number };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (currentContainer && gl.canvas) {
        // Check if the canvas is still a child before removing it
        if (currentContainer.contains(gl.canvas as Node)) {
             // FIX APPLIED HERE
            currentContainer.removeChild(gl.canvas as HTMLCanvasElement);
        }
      }
    };
